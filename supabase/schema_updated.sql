...
-- Jobs (updated with completion_notes and new status options)
CREATE TABLE jobs (
  ...
  status text DEFAULT 'open' CHECK (status IN ('open', 'in_progress', 'completed', 'cancelled', 'confirmed')),
...
-- Job conversations (NEW - for automatic messaging when application accepted)
...
-- Job updates (NEW - for progress reports and status updates)
...
-- Messages (updated with conversation_id)
CREATE TABLE messages (
  id uuid DEFAULT gen_random_uuid() PRIMARY KEY,
  job_id uuid REFERENCES jobs(id) ON DELETE CASCADE,
  conversation_id uuid REFERENCES job_conversations(id) ON DELETE CASCADE,
